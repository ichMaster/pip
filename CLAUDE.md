...
| [creature/pet.py](creature/pet.py) | `Pet` — orchestration; the only place body and mind meet. Splits the brain call for off-thread use: `snapshot()` / `think()` (pure, off-thread) / `apply_reply()` (main-thread, sole body writer); `respond()` is their sync composition. |
| [creature/chat.py](creature/chat.py) | `run()` — the real-time loop (tick → maybe speak → non-blocking input). Runs the brain **off-thread** (daemon worker + `queue`) so the ~1 s call never freezes the loop; shows a `thinking` line while pending, suppresses nags then. Face + bars shown **on demand** (`_show_status`); **no cursor-driven footer.** |
...
- **`Pet`** owns a `Needs` + a `Brain` and is the only place body and mind meet. `act(kind)` applies the numeric effects from the `ACTION_EFFECTS` table (`/feed` `/play` `/sleep`). The brain call is split for the async loop (v0.4): **`snapshot()`** copies the body (`Needs` + history) on the main thread; **`think(kind, text, snap)`** asks the mind *about that snapshot* — pure, no body mutation, so it's safe to run on a worker thread; **`apply_reply(kind, text, reply)`** folds the `mood` delta + (for chat) history back onto the live body — **main thread only, the body's single writer**. `respond()` is their synchronous composition (used by `--rule`/sync callers). This **single-thread-body invariant** is exactly what the v1.2 daemon reuses. Chat history keeps the last 8 turns; only the last 4 reach the LLM.

- **`run(pet)`** is the loop: tick the body by real `dt`, maybe emit a spontaneous line (rate-limited via `last_spontaneous`, scaled by `speed`), then non-blocking line input via `select.select(..., timeout=0.2)`. `_handle()` dispatches slash commands (returning `(action, payload)`); anything else becomes a `chat` turn. **The brain runs off the main thread (v0.4):** a chat/care turn snapshots the body and hands `pet.think` to a **daemon worker thread** (fed by a `queue`); the loop keeps ticking + polling while the reply is in flight, and applies it with `pet.apply_reply` (main thread) when the result lands — so the ~1 s LLM call **never freezes** the loop. While pending it's **THINKING**: a `thinking` line (`face(needs, state="thinking")` → `(✻⋅✻)`) is shown, **spontaneous nags are suppressed**, a second chat **queues (FIFO)**, and a care action's numeric effect applies immediately. EOF/`/quit` **drains** pending so no reply is lost; `Ctrl-C` exits at once (daemon worker). The face + bars are shown **on demand** by `_show_status` (printing `status_line`) — at startup, after care actions, and on `/status`; **no cursor-driven in-place footer** (the loop emits **zero cursor-control escapes**, so it cannot corrupt). **The two-clock split is unchanged — body mutated only on the main thread; this is presentation/scheduling only.** *(History: v0.1/v0.3 used a live in-place footer; it was dropped because the cursor arithmetic corrupted the screen on spontaneous lines and wrapped/emoji replies. A live, pinned, flicker-free status returns with the **v1.1** full-screen client — the only robust way to do it.)*

- **Rendering** is pure functions (`face_state`, `face_block`, `bar`, `status_line`, `welcome_banner`) driven entirely by `Needs`. `face_state` is a priority cascade (physical needs beat emotional; `happy` only when euphoric, `content` when merely comfortable, else `ok`). Faces are a Claude-adjacent minimal/sparkle set; every glyph must be **single-width** — `_assert_faces_single_width` runs at import (via `is_single_width`/`unicodedata.east_asian_width`, rejecting Wide/Full/Ambiguous) so a box-smearing glyph fails loudly. A transient **`thinking`** UI state is *not* need-derived (`face_state` never returns it); it's shown only via the optional `state=` override on `face`/`face_block`/`status_line` (default `None` = needs-derived), wired live in v0.4. ANSI codes degrade gracefully. `render.py` is the **sole owner of ANSI**. **Framing** (`box`, `columns`, `welcome_banner`) stays pure: the helpers take an explicit `width` and return strings; `chat.py` measures the terminal (`shutil.get_terminal_size`) and feeds width in. Width-aware `status_line(..., width=...)` stretches the bars to fit (`width=None` = fixed legacy layout). The cursor-control primitives (`cursor_up`, `save_cursor`, …) remain in `render.py` for the future v1.1 full-screen client but are **unused by the current loop**. Scrollback-preserving, **no `curses`/alternate-screen** (a full-frame client is v1.1).
