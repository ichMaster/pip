...
| [creature/render.py](creature/render.py) | Faces, bars, `status_panel` — pure functions of a `Needs`. Sole owner of ANSI/box-drawing. Faces are a minimal/sparkle set guarded single-width by `is_single_width` (`unicodedata.east_asian_width`). Framing primitives (`box`, `columns`, `welcome_banner`, `status_panel`) are width-fed and pure. (Cursor-control primitives — `cursor_up`, `clear_below`, `save_cursor`… — remain for the future v1.1 full-frame client but are unused by the current loop.) |
| [creature/pet.py](creature/pet.py) | `Pet` — orchestration; the only place body and mind meet. |
| [creature/chat.py](creature/chat.py) | `run()` — the real-time loop (tick → maybe speak → non-blocking input). Shows the face + bars in a framed `pip` panel **before** the chat (the welcome banner) and reprints it on `/status` and after care actions; conversation scrolls below. Measures the terminal (`shutil.get_terminal_size`) to size the banner. |
...
- **`run(pet)`** is the loop: tick the body by real `dt`, maybe emit a spontaneous line (rate-limited via `last_spontaneous`, scaled by `speed`), then non-blocking line input via `select.select(..., timeout=0.2)`. `_handle()` dispatches slash commands; anything else becomes a `chat` turn. The face + bars are a **framed `pip` panel** (`status_panel`) shown by `_draw_intro` as the welcome banner **before** the chat, and reprinted by `_show_status` on `/status` and after care actions; the conversation then scrolls normally below. There is **no in-place footer / cursor choreography** — a panel pinned at the top can't update live while chat scrolls past it without an alternate-screen TUI (that's the v1.1 client), so the status is a snapshot you refresh. This means **zero cursor-control escapes**, so the loop is robust (no half-drawn boxes during a slow synchronous LLM reply) and non-TTY output is automatically clean. **The two-clock split is unchanged — this is presentation only; the body is never touched here.** *(History note: v0.1/v0.3 used a live in-place footer; it was replaced by this top-panel snapshot because the in-place redraw corrupted the screen during ~1s blocking LLM calls. A live, flicker-free status returns with the v1.1 full-frame client.)*

- **Rendering** is pure functions (`face_state`, `face_block`, `bar`, `status_panel`, `welcome_banner`) driven entirely by `Needs`. `face_state` is a priority cascade (physical needs beat emotional; `happy` only when euphoric, `content` when merely comfortable, else `ok`). Faces are a Claude-adjacent minimal/sparkle set; every glyph must be **single-width** — `_assert_faces_single_width` runs at import (via `is_single_width`/`unicodedata.east_asian_width`, rejecting Wide/Full/Ambiguous) so a box-smearing glyph fails loudly. A transient **`thinking`** UI state is *not* need-derived (`face_state` never returns it); it's shown only via the optional `state=` override on `face`/`face_block`/`status_panel` (default `None` = needs-derived), wired live in v0.4. ANSI codes degrade gracefully. `render.py` is the **sole owner of ANSI**. **Framing** (`box`, `columns`, `welcome_banner`, `status_panel`) stays pure: the helpers take an explicit `width` and return strings; `chat.py` measures the terminal (`shutil.get_terminal_size`) and feeds width in. `box()` colors the **frame only**, so the colored bars survive inside a panel. Scrollback-preserving, **no `curses`/alternate-screen** (a full-frame client is v1.1).
