...
| [creature/chat.py](creature/chat.py) | `run()` — the real-time loop (tick → maybe speak → non-blocking input). Shows the face + bars **on demand** (`_show_status`): at startup, after care actions, and on `/status`; spontaneous lines just print. **No cursor-driven footer.** Measures the terminal (`shutil.get_terminal_size`) to size the banner/bars. |
...
- **`run(pet)`** is the loop: tick the body by real `dt`, maybe emit a spontaneous line (rate-limited via `last_spontaneous`, scaled by `speed`), then non-blocking line input via `select.select(..., timeout=0.2)`. `_handle()` dispatches slash commands; anything else becomes a `chat` turn. The face + bars are shown **on demand** by `_show_status` (printing `status_line`) — at startup (before the chat), after care actions, and on `/status`; pip's spontaneous lines just `print`. There is **no cursor-driven in-place footer**: redrawing a status block by moving the cursor is fragile — it drifts when a reply wraps, an emoji widens a line, or a spontaneous line lands mid-keystroke, leaving half-drawn boxes. So the loop emits **zero cursor-control escapes**: it cannot corrupt, and non-TTY output is automatically clean. **The two-clock split is unchanged — this is presentation only; the body is never touched here.** *(History: v0.1/v0.3 used a live in-place footer; it was dropped because the cursor arithmetic corrupted the screen on spontaneous lines and wrapped/emoji LLM replies. A live, pinned, flicker-free status returns with the **v1.1** full-screen client — the only robust way to do it.)*
