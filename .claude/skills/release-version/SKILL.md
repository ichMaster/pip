...
> Ported from the Lumi project and adapted to pip. This skill only touches **version metadata** — never the `creature/` source.
...
Examples:
- `/release-version 1.0.0` -- bump to 1.0.0, prompt for changelog
- `/release-version 1.2.0 Async mind off the body clock; Canon LLM chat over the socket` -- bump with provided changelog items
...
Versioning `v{major}.{phase}.{fix}` (see [spec/ROADMAP.md](../../../spec/ROADMAP.md)): `major` = track (v0 creature, v1 live-daemon, v2 front-desk), `phase` = milestone (minor), `fix` = patch. The current creature is `v0.0.x`. pip cuts a release per completed phase: a finished live-daemon phase cuts its release (`v1.2` done → `1.2.0`), and a follow-up fix bumps the patch (`1.2.1`). **Never change the version without explicit user confirmation.**
...
3. Summarize the changes into concise bullet points (group related commits; reference the pip phase `vA.B` where relevant)
...
- **Don't modify source files.** This skill only touches version metadata (VERSION, README.md, RELEASE.txt), never the `creature/` package (`needs`, `brain`, `render`, `pet`, `chat`, `cli`, or the planned `protocol`/`server`/`client`/`state`).
...
- **Ukrainian README.** pip's [README.md](../../../README.md) is Ukrainian — if it carries a version reference, update it in place without translating the rest.
